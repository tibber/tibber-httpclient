...
describe('pinoSerializers', () => {
  describe('req', () => {
    ...
    it('should redact url secrets from a successful request', () => {
      expect(req({ failed: false, method: 'GET', url: 'https://tok:pw@api.example.com/v1?api_key=abc' })).toBe(
        'GET https://redacted:redacted@api.example.com/v1?api_key=redacted',
      );
    });
  });
